package com.cse6324.http;

/**
 * Created by Jarvis on 2017/2/6.
 */

public class Constant {
    public final static String URL_REGISTER = "http://45.55.179.123/register.php?";
    public final static String URL_LOGIN = "http://45.55.179.123/login.php?";
    public final static String URL_RESETPASSWORD ="http://45.55.179.123/resetPassword.php?";
    public final static String URL_CALORIEPLAN ="http://45.55.179.123/calorieplan.php?";
    public final static String URL_VITALSIGNS = "http://45.55.179.123/vitalsigns.php?";

    public final static String URL_GETCONTACT = "http://45.55.179.123/getcontact.php?";
    public final static String URL_GETDEFAULTCONTACT = "http://45.55.179.123/getdefaultcontact.php?";
    public final static String URL_SETDEFAULTCONTACT = "http://45.55.179.123/setdefaultcontact.php?";
    public final static String URL_ADDCONTACT = "http://45.55.179.123/addcontact.php?";

    public final static String URL_SETNOTIFY = "http://45.55.179.123/setnotify.php?";

    public final static String URL_ADDNOTE = "http://45.55.179.123/addnote.php?";
    public final static String URL_GETNOTE = "http://45.55.179.123/getnote.php?";
    public final static String URL_UPDATENOTE = "http://45.55.179.123/updatenote.php?";

    public final static String URL_GETDIETHISTORY = "http://45.55.179.123/getdiethistory.php?";
    public final static String URL_ADDDIETHISTORY = "http://45.55.179.123/adddiethistory.php?";
    public final static String URL_DELETEDIETHISTORY = "http://45.55.179.123/deletediethistory.php?";

    public final static String URL_GETMEDICINE = "http://45.55.179.123/getmedicine.php?";
    public final static String URL_DELETEMEDICINE = "http://45.55.179.123/deletemedicine.php?";
    public final static String URL_ADDMEDICINE = "http://45.55.179.123/addmedicine.php?";
    public final static String URL_SETREMINDER = "http://45.55.179.123/setreminder.php?";

    public final static String URL_SEARCHFOOD = "https://api.nutritionix.com/v1_1/search?";
    public final static String URL_SEARCHMEDICINE = "https://clin-table-search.lhc.nlm.nih.gov/api/rxterms/v3/search?";

    public final static String URL_GETNEWNOTICE = "http://45.55.179.123/getnewnotice.php?";
    public final static String URL_GETNOTICE = "http://45.55.179.123/getnotice.php?";
    public final static String URL_DELETENOTICE = "http://45.55.179.123/deletenotice.php?";

    public final static String URL_SEARCH = "http://45.55.179.123/search.php?";

    public final static String URL_ARTICLE = "http://api.nytimes.com/svc/search/v2/articlesearch.json?";

    public final static String URL_CONFLICT = "http://45.55.179.123/page/conflict.php?";
    public final static String URL_SENDEMAIL = "http://45.55.179.123/sendemail.php?";
}